    ...
    return Scaffold(
      ...
      body: cart.items.isEmpty
          ? const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              SizedBox(height: 12),
              Text('Your cart is empty', style: TextStyle(fontSize: 16, color: AppTheme.textGrey)),
            ]))
          : Column(children: [
                  ...
                  child: Padding(
                    ...
                    child: Row(children: [
                      ClipRRect(
                        ...
                        child: ProductImage(imageUrl: item.product.imageUrl, width: 80, height: 80, fit: BoxFit.cover),
                      ),
                      ...
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Row(children: [
                          ...
                          const SizedBox(width: 12),
                          Text('${item.quantity}', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
                          const SizedBox(width: 12),
                          GestureDetector(
                            onTap: () => cart.updateQty(item.product.id, item.quantity + 1),
                            child: Container(width: 28, height: 28,
                ...
                Container(
                  ...
                  child: Column(children: [
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      const Text('Total MRP', style: TextStyle(fontSize: 13, color: AppTheme.textGrey)),
                      Text('₹${cart.items.fold(0, (s, i) => s + (i.product.originalPrice * i.quantity).toInt())}',
                          style: const TextStyle(fontSize: 13)),
                    ]),
                    const SizedBox(height: 4),
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      const Text('Discount', style: TextStyle(fontSize: 13, color: AppTheme.textGrey)),
                      Text('-₹${cart.items.fold(0, (s, i) => s + ((i.product.originalPrice - i.product.price) * i.quantity).toInt())}',
                          style: const TextStyle(fontSize: 13, color: Colors.green)),
                    ]),
                    const SizedBox(height: 4),
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      const Text('Delivery', style: TextStyle(fontSize: 13, color: AppTheme.textGrey)),
                      const Text('FREE', style: TextStyle(fontSize: 13, color: Colors.green)),
                    ]),
...
class _CheckoutSheetState extends State<CheckoutSheet> {
  ...
  Future<void> _fetchCurrentLocation() async {
    ...
    try {
      ...
      if (permission == LocationPermission.deniedForever) {
        setState(() => _error = 'Enable location from phone settings');
        return;
      }
  ...
  void _showUpiDialog() {
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: () { Navigator.pop(ctx); _submitOrder(); },
    ...
    try {
      final orderRes = await http.post(
        ...
        body: jsonEncode({'items': widget.cart.items.map((i) => {
          'product_id': i.product.id, 'qty': i.quantity, 'price': i.product.price
        }).toList()}),
      );
      if (orderRes.statusCode != 201) {
        setState(() { _error = 'Failed to create order.'; _processing = false; });
        return;
      }
      ...
      if (intentRes.statusCode != 200) {
        setState(() { _error = 'Payment setup failed.'; _processing = false; });
        return;
      }
  ...
  Future<void> _submitOrder() async {
    ...
    try {
      ...
      if (mounted) setState(() { _error = response.body; _processing = false; });
        return;
      }
      ...
      builder: (_, sc) => Container(
        ...
        child: Column(children: [
          ...
          Padding(padding: const EdgeInsets.all(16), child: Row(children: [
            ...
            if (_step == 0) ...[
              ...
              TextButton.icon(
                ...
                label: Text(
                  _loadingLocation ? 'Fetching...' : 'Use Location',
          ...
          Expanded(child: SingleChildScrollView(controller: sc, padding: const EdgeInsets.all(16),
              child: _step == 0 ? _buildAddressForm() : _buildOrderSummary())),
          Padding(padding: const EdgeInsets.fromLTRB(16, 8, 16, 24), child: SizedBox(width: double.infinity,
  ...
  Widget _buildOrderSummary() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      ...cart.items.map((item) => Container(
        ...
        child: Row(children: [
          ClipRRect(borderRadius: BorderRadius.circular(6),
              child: item.product.imageUrl.startsWith('http') ? Image.network(item.product.imageUrl,) : Image.asset(item.product.imageUrl, width: 60, height: 60, fit: BoxFit.cover,
      ...
      Container(padding: const EdgeInsets.all(14),
          ...
          child: Column(children: [
            _priceRow('Total MRP', '₹${cart.items.fold(0, (s, i) => s + (i.product.originalPrice * i.quantity).toInt())}', Colors.black87),
            _priceRow('Discount', '-₹${savings.toInt()}', Colors.green),
            _priceRow('Delivery', 'FREE', Colors.green),
            const Divider(height: 16),
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              const Text('Total Payable', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 15)),
              Text('₹${cart.total.toInt()}', style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 18, color: AppTheme.primary)),
            ]),
          ])),
      const SizedBox(height: 16),
      const Text('Payment Method', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
      const SizedBox(height: 8),
      _payOpt('upi', Icons.account_balance_wallet_outlined, 'UPI / Net Banking'),
      _payOpt('card', Icons.credit_card_outlined, 'Debit / Credit Card (Stripe)'),
      _payOpt('cod', Icons.money_outlined, 'Cash on Delivery'),
      if (_error != null) Padding(padding: const EdgeInsets.only(top: 8),
          child: Text(_error!, style: const TextStyle(color: Colors.red))),
    ]);
  }

  Widget _payOpt(String val, IconData icon, String label) {
    final sel = _paymentMethod == val;
    return GestureDetector(onTap: () => setState(() => _paymentMethod = val),
      child: Container(margin: const EdgeInsets.only(bottom: 8),
          ...
          decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(8),
              border: Border.all(color: sel ? AppTheme.primary : Colors.grey.shade200, width: sel ? 1.5 : 1)),
          child: Row(children: [
            Icon(icon, color: sel ? AppTheme.primary : Colors.grey, size: 20),
            const SizedBox(width: 10),
  ...
  Widget _tf(String label, TextEditingController ctrl, TextInputType type, {int? max}) => Padding(
